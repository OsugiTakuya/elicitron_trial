...
EVAL_SYSTEM_MSG = "あなたは包括的な要件文書を作成するための情報の十分性を評価する専門家です。"
EVAL_HUMAN_PREFIX_MSG = "以下のユーザーリクエストとインタビュー結果に基づいて、包括的な要件文書を作成するのに十分な情報が集まったかどうかを判断してください。"
DOC_GEN_SYSTEM_MSG = "あなたは収集した情報に基づいて要件文書を作成する専門家です。"
DOC_GEN_HUMAN_PREFIX_MSG = "以下のユーザーリクエストと複数のペルソナからのインタビュー結果に基づいて、要件文書を作成してください。"
DOC_GEN_HUMAN_POSTFIX_MSG = (
...
class InformationEvaluator:
    def __init__(
        self,
        llm: ChatOpenAI,
        system_msg: str = EVAL_SYSTEM_MSG,
        human_prefix_msg: str = EVAL_HUMAN_PREFIX_MSG,
    ):
        self.llm = llm.with_structured_output(EvaluationResult)
        self.system_msg = system_msg
        self.human_prefix_msg = human_prefix_msg
...
class RequirementsDocumentGenerator:
    def __init__(
        self,
        llm: ChatOpenAI,
        system_msg: str = DOC_GEN_SYSTEM_MSG,
        human_prefix_msg: str = DOC_GEN_HUMAN_PREFIX_MSG,
        human_postfix_msg: str = DOC_GEN_HUMAN_POSTFIX_MSG,
    ):
        self.llm = llm
        self.system_msg = system_msg
        self.human_prefix_msg = human_prefix_msg
        self.human_postfix_msg = human_postfix_msg
...
class DocumentationAgent:
    def __init__(
        ...
        llm: ChatOpenAI,
        k: Optional[int] = None,
        eval_system_msg: str = EVAL_SYSTEM_MSG,
        eval_human_prefix_msg: str = EVAL_HUMAN_PREFIX_MSG,
        doc_gen_system_msg: str = DOC_GEN_SYSTEM_MSG,
        doc_gen_human_prefix_msg: str = DOC_GEN_HUMAN_PREFIX_MSG,
        doc_gen_human_postfix_msg: str = DOC_GEN_HUMAN_POSTFIX_MSG,
    ):
        # 各種ジェネレータの初期化
        ...
        self.information_evaluator = InformationEvaluator(
            ...
            human_prefix_msg=eval_human_prefix_msg,
        )
        self.requirements_generator = RequirementsDocumentGenerator(
            llm=llm,
            system_msg=doc_gen_system_msg,
            human_prefix_msg=doc_gen_human_prefix_msg,
            human_postfix_msg=doc_gen_human_postfix_msg,
        )
